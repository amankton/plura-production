...
const main = async () => {
  ...
  const sealedEvidencePath = path.join(
    evidenceDirectory,
    'CF-P1-B4F1-dependency-evidence.json'
  )
  const sealedEvidence = JSON.parse(
    await Bun.file(sealedEvidencePath).text()
  ) as {
    audit: {
  ...
  const outage = await Bun.file(
    path.join(evidenceDirectory, 'CF-P1-B4F2A1-audit-outage.json')
  ).text()
  ...
  const summary = {
    ...
    artifacts: {
      auditOutage: {
        path: 'docs/evidence/CF-P1-B4F2A1-audit-outage.json',
        sha256: hash(outage),
      },
      installedDependencyInventory: {
        path: 'docs/evidence/CF-P1-B4F2A1-sbom.txt',
  ...
  await Promise.all([
    Bun.write(
      path.join(evidenceDirectory, 'CF-P1-B4F2A1-sbom.txt'),
      inventory
    ),
    Bun.write(
      path.join(evidenceDirectory, 'CF-P1-B4F2A1-dependency-evidence.json'),
      `${JSON.stringify(summary, null, 2)}\n`
    ),
  ])

  console.log(
    `PASS provisional same-lock evidence: audit STALE_UNREVALIDATED; SBOM SHA-256 ${summary.artifacts.installedDependencyInventory.sha256}`
  )
}
