...
$schemaFixturePath = Join-Path $repositoryRoot 'tests/fixtures/mysql/webhook-processing-schema.sql'
$inboxDdlPath = Join-Path $repositoryRoot 'docs/execution/sql/CF-P1-B4F1-webhook-inbox-foundation.sql'
$databaseName = 'crewframe_b4f2a2'
$containerPrefix = 'crewframe-b4f2a2-'
...
function Invoke-BunScenario {
  param(
    ...
    [Parameter(Mandatory = $true)][ValidateSet('missing', 'outage', 'success')][string]$Scenario,
    [string]$ContainerName
  )

  $previousDatabaseUrl = $env:DATABASE_URL
  $previousContainerName = $env:CREWFRAME_MYSQL_CONTAINER
  try {
    $env:DATABASE_URL = $DatabaseUrl
    if ($ContainerName) {
      $env:CREWFRAME_MYSQL_CONTAINER = $ContainerName
    } else {
      Remove-Item Env:CREWFRAME_MYSQL_CONTAINER -ErrorAction SilentlyContinue
    }
    bun scripts/verify-webhook-processing-store.ts $Scenario
    ...
    if ($null -eq $previousContainerName) {
      ...
      $env:CREWFRAME_MYSQL_CONTAINER = $previousContainerName
    }
  }
}

function Invoke-DatabaseProof {
  $suffix = [Guid]::NewGuid().ToString('N').Substring(0, 10)
  $containerName = "$containerPrefix" + "proof-$suffix"
  ...
  try {
    ...
    $port = $Matches[1]
    $databaseUrl = "mysql://root:$password@127.0.0.1:$port/$databaseName`?connection_limit=40&connect_timeout=3&pool_timeout=5"

    Invoke-BunScenario -DatabaseUrl $databaseUrl -Scenario missing
    Invoke-MySql -ContainerName $containerName -Password $password `
      -Sql (Get-Content -Raw -LiteralPath $schemaFixturePath)
    Invoke-MySql -ContainerName $containerName -Password $password `
      -Sql (Get-Content -Raw -LiteralPath $inboxDdlPath)
    Invoke-BunScenario -DatabaseUrl $databaseUrl -Scenario success `
...
if ($remaining.Count -ne 0) {
  throw "Disposable B4F2A2 containers remain: $($remaining -join ', ')"
}
